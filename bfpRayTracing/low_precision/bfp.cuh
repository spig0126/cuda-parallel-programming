...
bfpNum float_to_bfpNum(float f) {
    ...
    if (*t)
        res.mant ^= 0x00800000; // add implicit 1 in mantissa only if value is not 0
        
    // set bfp's mantissa from float's extracted mantissa bits
    if (FLOAT_MANT_BITSIZE >=
        BFP_MANT_BITSIZE) { // if bfp mantissa is smaller in bit size than float
                            // mantissa
    ...
    if (FLOAT_EXP_BITSIZE >= BFP_EXP_BITSIZE) { // if bfp exponent is smaller in
