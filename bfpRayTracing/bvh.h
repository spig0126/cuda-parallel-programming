...
class bvh_node : public hittable {
  public:
    ...
    bvh_node(const hittable_list &list, float time0, float time1)
        : bvh_node(1, list.objects, 0, list.objects.size(), time0,
                   time1) // 1: root node의 nodeIdx값
    {}

    bvh_node(int idx, const std::vector<shared_ptr<hittable>> &src_objects,
...
bvh_node::bvh_node(int idx,
    ...
    if (object_span == 1) {
        ...
        left =
            make_shared<bvh_node>(idx + 1, objects, start, mid, time0, time1);
    ...
    if (!left->bounding_box(
            time0, time1,
            box_left) // 자식 노드의 aabb를 box_left에 저장하는 부분
    ...
    // ------------------------------\n", nodeIdx, object_span); printf(" -%-8s:
    // (%d ~ %d)\n", "Objects", (int)start, (int)end-1); printf("  -%-8s:
    // (%.1lf,%.1lf,%.1lf) ~ (%.1lf,%.1lf,%.1lf)\n", "AABB", pm.e[0], pm.e[1],
    // pm.e[2], pM.e[0], pM.e[1], pM.e[2]);
}
